module Keeper_excusals
  def self.attachment_for_main_screen
    [
      {
  ...
  def self.add_option__block(today_now:, user_id:)
    {
  ...
  def self.list_block(team_id:, page:)
    json_blocks = []
  ...
  def values_for_buttons(type_of_command:, recent_value:)
    ...
    when 'list'
      ...
      right =
        if maximal_iteration <= (recent_value + 1) * $iteration
          0
        else
          recent_value + 1
        end
      right_caption = 'Następny'
      left = if recent_value - 1 < 0
    ...
    when 'help'
      little_organization = [%w[commands morning_stand evening_stand],
  ...
  def direction_buttons(type_of_command:, recent_value:)
    callback = if type_of_command.eql?('list')
                 'choice_for_excusal'
               else
                 'help'
               end
