class EverythingNeeded
  ...
  def is_bot(user_id:, team_id:)
    if @real_user[user_id].nil? && !($everything_needed.get_slack_client(team_id: team_id).users_info(user: user_id)[:user][:is_bot]) && !($everything_needed.get_slack_client(team_id: team_id).users_info(user: user_id)[:user][:is_bot])
